...
app.directive('chardataSelect',['chartData', function (chartData) {
	return {
		restrict: 'AE',
		link: function (scope, elem, attrs) {
			elem.on('click', function () {
				chartData.usingData[attrs.chartdataName] = !chartData.usingData[attrs.chartdataName];
				chartData.usingData[attrs.chartdataType]= !chartData.usingData[attrs.chartdataType];
			});
		}
	};
}]);
//tr可以选中
app.directive('trSelect',['chartData', function (chartData) {
	return {
		restrict: 'AE',
		link: function (scope, elem, attrs) {
			elem.on('click', function () {
				elem.toggleClass('info');
				chartData.usingData.dataID = elem.children('td:eq(1)').text();
